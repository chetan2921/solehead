...
class _FollowersScreenState extends State<FollowersScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<UserProvider>().loadUserFollowers(widget.userId);
    });
  }

  Future<void> _refreshFollowers() async {
    await context.read<UserProvider>().loadUserFollowers(widget.userId);
  }
      ...
      body: Consumer<UserProvider>(
        builder: (context, userProvider, child) {
          if (userProvider.isLoading && userProvider.followers.isEmpty) {
            return const Center(
              child: widgets.LoadingWidget(message: 'Loading followers...'),
            );
          }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            widget.isCurrentUser
                ? 'Share your sneaker posts to gain followers!'
                : 'Be the first to follow ${widget.username}!',
  ...
  Future<void> _toggleFollow(String userId, bool isCurrentlyFollowing) async {
    ...
    if (success) {
      // Refresh the followers list to get updated data
